...
//! Perform a retrieval from a basic hash table.  Each thread manages a single query.
template <unsigned kNumHashFunctions> __global__
void retrieve_hash_join(const unsigned                 n_queries,
                   const unsigned                     *keys_in,
                   const unsigned                      table_size,
                   const Entry                        *table,
                   const Functions<kNumHashFunctions>  constants,
                   ...
                   const unsigned                      stash_count,
                         unsigned                     *out_query,
                         unsigned                     *out_table,
                         unsigned                     *block_result_num,
                         unsigned                     *num_probes_required,
                             bool                      anti)
{

  typedef cub::BlockScan<unsigned, kBlockSize> BlockScan;

  bool semiJoin = (out_table == NULL);

  // Allocate shared memory for BlockScan
  __shared__ typename BlockScan::TempStorage temp_storage;
  __shared__ unsigned s_lookup_value[kBlockSize];
  __shared__ unsigned s_row_value[kBlockSize];

  unsigned block_index = blockIdx.y * gridDim.x + blockIdx.x;
  unsigned thread_index = block_index * blockDim.x + threadIdx.x;

  unsigned key;
  unsigned lookup_value = kNotFound;
  unsigned flag = 0;
  unsigned scan = 0;

  if(thread_index < n_queries) {
      key = keys_in[thread_index];
      lookup_value = retrieve<kNumHashFunctions>
                                     (key,
                                      table_size,
                                      table,
                                      constants,
  ...
  BlockScan(temp_storage).ExclusiveSum(flag, scan);

  if(flag) {
      s_lookup_value[scan] = lookup_value;
      s_row_value[scan] = thread_index;
  }

  __syncthreads();

  if(thread_index < n_queries) {
      //only write output for table values if full join
      if(!semiJoin) {
        out_table[thread_index] = s_lookup_value[threadIdx.x];
      }
      out_query[thread_index] = s_row_value[threadIdx.x];
  }

  if(threadIdx.x == (blockDim.x - 1) ) {
      block_result_num[blockIdx.y * gridDim.x + blockIdx.x] = scan + flag;
  }

  //if(threadIdx.x == 0 && blockIdx.x == 0 && blockIdx.y == 0) 
  //  printf("FROM GPU - probe found at index: 0, key: %i, lookup_value: %i \n", key, lookup_value);
}


__global__ void block_compaction(unsigned    n_queries, 
                                 unsigned   *d_block_result_sizes, 
                                 unsigned   *d_block_scans, 
                                 unsigned   *d_temp_query, 
                                 unsigned   *d_temp_table, 
                                 unsigned   *d_out_query, 
    ...
    unsigned block_result_size = d_block_result_sizes[block_index];
    unsigned write_pos = d_block_scans[block_index];


    if(threadIdx.x < block_result_size) {
        
        write_pos += threadIdx.x;

        d_out_query[write_pos] = d_temp_query[thread_index];

        if(d_out_table != NULL) {
          d_out_table[write_pos] = d_temp_table[thread_index];
        }
...
//! Perform a retrieval of match bitmap from a basic hash table
template <unsigned kNumHashFunctions> __global__
void retrieve_bitmap_semi_join(const unsigned                 n_queries,
                          const unsigned                     *keys_in,
                          const unsigned                      table_size,
                          const Entry                        *table,
                          const Functions<kNumHashFunctions>  constants,
...
bool insert(const unsigned                      table_size,
  ...
  //store original key for duplicate elimination
  unsigned insert_key = get_key(entry);
  unsigned key = insert_key;
  ...
  for (unsigned its = 1; its <= max_iteration_attempts; its++) {
    ...
    // If no key was evicted or key is duplicate, we're done.
    if (key == kKeyEmpty) {
      *iterations_used = its;
      break;
    }
...
void CuckooHash(const unsigned   n_entries,
  ...
  if(values == NULL) entry = make_entry(keys[thread_index], 1);
  else entry = make_entry(keys[thread_index], values[thread_index]);

  //if(keys[thread_index] == 193)
  //   printf("FROM GPU - found key 193 in build\n");
